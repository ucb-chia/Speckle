#!/bin/bash
# On-target driver for one CINT2006 benchmark. Mirrors the spec17
# intspeed.sh + intrate.sh runners: supports a single-process run with
# optional OpenMP threads (`--threads N`), or N-copy "rate-style" parallel
# runs (`--copies N`). SPEC06 has no speed/rate suite split — these are
# just two ways to drive the same binary.
#
# Output: ~/output/<bench>{_<copy>}.{out,err,csv}, plus TMA before/after
# snapshots when a tma_reader is present alongside this script.
...
function usage {
    cat <<EOF
usage: cint.sh <benchmark> [options]
  benchmark      a CINT2006 run dir under \$PWD (e.g. 400.perlbench)
...
if [ "${copies}" -gt 1 ]; then
    # Rate-style: N concurrent copies, no per-workload selection.
    ...
    # Single-process speed-style run.
    ...
    # busybox's `time` mishandles \n in -f; write the header manually.
